...
public class DefaultActorService : IActorService
{
    ...
    public async Task<Result<PagedResult<Actor>>> ReadActors(int page, int size)
    {
        ...
        ? new Result<PagedResult<Actor>>(new Exception($"Could not read actors from page {page} and size {size}."),
    ...
    public async Task<Result<Actor>> CreateActor(Actor newActor)
    {
        ...
        ? new Result<Actor>(new Exception($"Could not create actor {newActor}."),
        (int)HttpStatusCode.NotFound): new Result<Actor>(actor, (int)HttpStatusCode.Created);
    ...
    public async Task<Result<Actor>> ReadActor(int id)
    {
        ...
        (int)HttpStatusCode.NotFound): new Result<Actor>(actor, (int)HttpStatusCode.OK);
    ...
    public async Task<Result<Actor>> DeleteActor(int id)
    {
        ...
        var result = actor == null
        ? new Result<Actor>(new Exception($"Could not delete actor with id {id}."),
        (int)HttpStatusCode.NotFound): new Result<Actor>(actor, (int)HttpStatusCode.OK);
    ...
    private static Result<Actor>? ValidateActor(Actor? actorData)
    {
        ...
        {
            return new Result<Actor>(
            new Exception("Actor cannot be longer than 256 characters."),
            (int)HttpStatusCode.BadRequest);
        }

        if (actorData.YearBirth < 1888 || actorData.YearBirth > DateTime.UtcNow.Year)
        {
            return new Result<Actor>(
